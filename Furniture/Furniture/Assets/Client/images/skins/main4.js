...
$(function () {
    ...
    //alert(listimg.length);
    if (listimg.length > 1) {
        $.getScript("skins/simple-lightbox.js").done(function (script, textStatus) {
            for (var i = 0; i < listimg.length; i++) {
                var getimg = "<li>" + listimg[i].outerHTML + "</li>";
                var urlcurent = listimg[i].src;
                var altcurent = listimg[i].src;
            ...
            $(".divdetail img").click(function () {
                ...
                for (var i = 0; i < tongnut; i++) {
                    var checkurl = $('.gallery a:eq(' + i + ')').attr("href").replace("{WEBLINK}", "");
                    if (checkurl == chiso) {
